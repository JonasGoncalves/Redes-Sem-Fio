...
def Mensagem(Tipo, Msg1, Msg2):
	...
	if(Tipo == 10):
		...
		Msg = "\nMAC: "+str(Msg1)+"| (Recebe_Request) Enviando response\n"
		Script(Msg)
	elif(Tipo == 53):
		Msg = "\nMAC: "+str(Msg1)+"| (Recebe_Request) Repassando pacote\n"
		Script(Msg)
	elif(Tipo == 60):
		Msg = "\nMAC: "+str(Msg1)+"| (Envia_Response) Enviando response para "+str(Msg2)+"\n"
		Script(Msg)
	elif(Tipo == 61):
		Msg = "\nMAC: "+str(Msg1)+"| (Envia_Response) Nenhuma rota encontrada para "+str(Msg2)+". Enviando Route Request!\n"
		Script(Msg)
	elif(Tipo == 62):
		Msg = "\nMAC: "+str(Msg1)+"| (Envia_Response) Rota encontrada. Next_step = "+str(Msg2)+"\n"
		Script(Msg)
	elif(Tipo == 70):
		Msg = "\nMAC: "+str(Msg1)+"| (Recebe_Response) Pacote response com destino para "+str(Msg2)+"\n"
		Script(Msg)
	elif(Tipo == 71):
		Msg = "\nMAC: "+str(Msg1)+"| (Recebe_Response) Pacote descartado (nSeq)\n"
		Script(Msg)
	elif(Tipo == 72):
		Msg = "\nMAC: "+str(Msg1)+"| (Recebe_Response) Pacote recebido\n"
		Script(Msg)
	elif(Tipo == 73):
		Msg = "\nMAC: "+str(Msg1)+"| (Recebe_Response) Encaminhando pacote para"+str(Msg2)+"\n"
		Script(Msg)
	elif(Tipo == 80):
...
class Camada_Rede(object):
	...
	def Envia_Pacote(self, Pacote):
		...
		self.Camada_Enlace.Envia_Pacote(Pacote)
	...
	def Recebe_Request(self, PacReq):
		...
		if nSeq in self.nSeq:
			...
			#Envia response caso seja o destino do request
			if(Destino == self.MAC):
				...
				#Envia um pacote response
				self.Envia_Response(Origem, Dados)
	...
	#6X		
	def Envia_Response(self, Destino, Dados):
		...
		#Cria um pacote response
		Pacote_RRep = Pacote_Roteamento(self.MAC, Destino, Next, nSeq, "RRep", Dados)

		#Envia o pacote para o enlace
		self.Envia_Pacote(Pacote_RRep)

	#7X
	def Recebe_Response(self, PacResp):
	...
	#Libera um pacote de acordo com o MAC
	...
	#Dado um destino, verifica o proximo passo
	...
	#Mantem apenas as melhores rotas
	...
	#Formata a rota de acordo com a tabela
